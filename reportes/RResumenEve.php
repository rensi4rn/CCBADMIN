...
class RResumenEve extends  ReportePDF {
	var $datos_titulo;
	var $datos_detalle;
	var $ancho_hoja;
	var $gerencia;
   ...
   function generarReporte() {
	...
	function imprimirLinea($val,$count,$fill){	
		...
		$newDate = date("d/m/Y", strtotime( $val['fecha_programada']));
		$casa = "\t\t\t\t\t\t".$val['nombre_co'];
		
		if(isset($val['desc_obrero']) && $val['desc_obrero'] != ''){
			$casa =  $casa.' ('.$val['desc_obrero'].')';
		}
		...
		$RowArray = array(
                        ...
                        'casa' => $casa,
                        's1' => $newDate,
                        's2' => intval($val['cantidad_hermano']),
                        's3' => intval($val['cantidad_hermana']),
                        's4' => intval($val['cantidad_hermano'] + $val['cantidad_hermana']));
